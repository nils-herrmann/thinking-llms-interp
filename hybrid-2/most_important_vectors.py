...
def _analyze_file(path: str, *, top_k: Optional[int]) -> Dict[str, object]:
    ...
    assert num_records > 0, f"No records found in {path}"
...
def _find_rolling_files(rolling_dir: str) -> List[str]:
    files = []
    for name in os.listdir(rolling_dir):
        ...
        if not name.startswith("rolling_"):
            continue
        files.append(os.path.join(rolling_dir, name))
    files.sort()
    return files
...
def main() -> None:
    ...
    files = _find_rolling_files(rolling_dir)
    assert len(files) > 0, f"No rolling files in {rolling_dir}"

    print(f"Scanning {len(files)} rolling files in {rolling_dir}\n")
    for fp in files:
        stats = _analyze_file(fp, top_k=args.top_k)
        print(f"== {stats['file']} ==")
        ...
        if args.save_json:
            out_path = fp.replace(".jsonl", "_vector_stats.json")
