...
def custom_generate_with_projection_removal(model, tokenizer, input_ids, max_new_tokens, label, feature_vectors, layers=[10], coefficient=0.1, steer_positive=False, show_progress=True):
    ...
    if label in feature_vectors:
        ...
        feature_vector = None
        print(f"Feature vector for label {label} not found")
            ...
            with model.trace(input_chunk) as trace:
                ...
                if feature_vector is not None:
                    for layer_idx in layers:
                        ...
                        if steer_positive:
                            expanded_feature = feature_vector[layer_idx].unsqueeze(0)
                            projection = torch.einsum('bh,bh->b', model.model.layers[layer_idx].output[0][:, -1], expanded_feature)
                            projection_vector = projection.unsqueeze(-1).abs() * expanded_feature
